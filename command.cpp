  ...
  while ((c = Extra_UtilGetopt(argc, argv, "isrh")) != EOF) {
    ...
    case 'r':
      if (globalUtilOptind >= argc) {
        Abc_Print(
            -1,
            "Command line switch \"-r\" should be followed by an string.\n");
        goto usage;
      }
      fRedirect = true;
...
int SampleCkt_Command(Abc_Frame_t *pAbc, int argc, char **argv) {
  ...
  case 0:
    if (fCorrelation)
      pAig = sc.genCircuit(pNtk, fVerbose, true);
    else
...
usage:
  ...
  Abc_Print(-2, "\t-t <num>  : set the sampling type(0:xGen, 1:rand, "
                "2:randCof)[default:0]\n");
  Abc_Print(
      -2,
      "\t-c        : consider supports information of the current network\n");
...
usage:
  ...
  Abc_Print(-2,
            "\t        Using created sampling circuit to generate samples\n");
...
// ABC command: Generate a sampling circuit and connect it to the current
// network
...
usage:
  ...
  Abc_Print(-2, "\t        Generate a sampling circuit with given PI number "
                "and connect it to the current network\n");
  Abc_Print(-2, "\t-i <num> : set the number of PI\n");
  Abc_Print(-2, "\t-t <num> : set the sampling type(0:xGen, 1:rand, "
                "2:randCof)[default:0]\n");
  Abc_Print(
...
static double calMean(vector<double> &res) {
  double sum = 0;
  for (double d : res) {
    sum += d;
  }
...
static double calVar(vector<double> &res) {
  double mean = calMean(res);
  double sum = 0;
  for (double d : res) {
    sum += pow(d - mean, 2);
  }
...
static double calMin(vector<double> &res) {
  double min = DBL_MAX;
  for (double d : res) {
    if (d < min) {
      min = d;
...
int SampleChiTest_Command(Abc_Frame_t *pAbc, int argc, char **argv) {
  ...
  for (int iter = 0; iter < 100; ++iter) {
    ...
    if (fVerbose)
      Abc_Print(-2,
                "\nConduct Pearson's chi-squared test w/ nPI = %d, nPO = %d, "
                "and nSample = %d.\n",
                nPI, nPO, nSample);
    pInput = new int[nPI];
    while (nGen < nSample) {
  ...
  std::cout << "Average P-value = " << calMean(res) << std::endl;
  std::cout << "Min P-value = " << calMin(res) << std::endl;
  std::cout << "Variance of P-value = " << calVar(res) << std::endl;

  return 0;

usage:
  Abc_Print(-2, "usage: sampleChiTest [-i <num>] [-e <num>] [-o <num>/-c] [-r "
                "<file>] [-vh]\n");
  Abc_Print(-2,
            "\t        Apply Pearson Chi-Square test on sampling circuit\n");
  Abc_Print(-2, "\t-i <num>  : set the number of PI\n");
  Abc_Print(-2, "\t-o <num>  : set the number of PO\n");
...
int SampleStuckTest_Command(Abc_Frame_t *pAbc, int argc, char **argv) {
  ...
  if (fVerbose) {
    ...
    Abc_Print(2, "Avg.%% Hit in origin: %.1f\n",
              100 * accumulate(vCount1.begin(), vCount1.end(), 0.0) / nTest /
                  nSample);
    Abc_Print(2, "Avg.%% Dup in origin: %f\n",
              100 * accumulate(vDup1.begin(), vDup1.end(), 0.0) / nTest /
                  nSample);
    Abc_Print(2, "Avg. Fixed variables: %.1f\n",
              accumulate(vNFixed.begin(), vNFixed.end(), 0.0) / nTest);
    Abc_Print(2, "Avg.%% Hit in biased: %.1f\n",
              100 * accumulate(vCount2.begin(), vCount2.end(), 0.0) / nTest /
                  nSample);
    Abc_Print(2, "Avg.%% Dup in biased: %f\n",
              100 * accumulate(vDup2.begin(), vDup2.end(), 0.0) / nTest /
                  nSample);
  }
...
usage:
  Abc_Print(-2, "usage: sampleStuckTest [-i <num>] [-s <num>] [-n <num>] [-r "
                "<file>] [-dvh]\n");
  Abc_Print(-2, "\t        Generate single stuck-at fault on current network "
                "and test bias sampling\n");
  ...
  Abc_Print(-2, "\t-d        : toggles whether to dump the aig "
                "circuits[default: False]\n");
...
int SampleEffectiveTest_Command(Abc_Frame_t *pAbc, int argc, char **argv) {
  char c;
  bool fUseXor = false;
  int fIter = 100;
  Abc_Ntk_t *pNtk;
  ...
  // parse arguments
  Extra_UtilGetoptReset();
  while ((c = Extra_UtilGetopt(argc, argv, "ix")) != EOF) {
    switch (c) {
    case 'x':
      fUseXor = true;
      break;
  ...
  // get the current network
  pNtk = Abc_FrameReadNtk(pAbc);
  pSim = new SimTest(pNtk, fUseXor);
  pSim->SimTestExec();
  delete (pSim);
  return 0;

usage:
  Abc_Print(-2, "usage: sampleEffectTest [-i <num>] [-xh]\n");
}
